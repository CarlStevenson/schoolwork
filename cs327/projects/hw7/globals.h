...
/**
 * List of statements to be written
 */
typedef struct write_list {
  expr *arg;                /**< The first expression to write.     */
  expr *field_width;        /**< The field width of the expression. */
  struct write_list *next;  /**< The remainder of the list.         */
} write_list;
/**
 * Write statement.
 */
typedef struct {
  write_list *arg;  /**< The argument. */
} write_stmt;
 ...
 */
typedef struct {
  expr *arg;              /**< The argument to be assigned a value. */
  struct read_list *next; /**< The remainder of the list.           */
} read_list;
 ...
 */
typedef enum {
  S_IF, S_IF_ELSE, S_WHILE, S_CMPD, S_ASSIGN, S_UNARY, S_WRITE, S_WRITELN, S_READ, S_EMPTY
} stmt_tag;
...
extern write_list *write_list_new(expr *arg, expr *field_width, write_list *next);
extern write_stmt *write_stmt_new(write_list *arg);
extern writeln_stmt *writeln_stmt_new(write_list *arg);
extern read_list *read_list_new(expr *arg, read_list *next);
extern read_stmt *read_stmt_new(read_list *arg);
...
typedef union {
  ...
  write_list *write_list;            /**< List of expressions to be written.         */
  write_stmt *write_stmt;            /**< A write statement.                         */
  writeln_stmt *writeln_stmt;        /**< A writeln statement.                       */
  read_list *read_list;              /**< List of variables to be assigned variables.*/
  read_stmt *read_stmt;              /**< A read statement.                          */
...
typedef enum opcode {
  ...
  O_INCRE, O_DECRE, O_UPLUS, O_UMINUS, O_NOT, O_READ, O_FALSE, O_TRUE
} opcode;
