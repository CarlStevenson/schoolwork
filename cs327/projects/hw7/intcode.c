...
symbol *binary_expr_gen(binary_expr *e) {
  ...
  if(typecheck(e->lrand)!=typecheck(e->rrand)){
 ...
 */
void bool_const_expr_gen(bool_const_expr *e, symbol *true, symbol *false) {
...
void bool_unary_expr_gen(unary_expr *e, symbol*true, symbol *false){
  switch(e->op){
    case U_NOT:
          bool_expr_gen(e->rand,false,true);
...
void bool_expr_gen(expr *e, symbol *true, symbol *false) {
  if (next == MAX_QUADS) error("Quad table size exceeded");
  switch(e->tag) {
  case E_BOOL_CONST: return bool_const_expr_gen(e->bool_const, true, false); break;
  case E_VARIABLE: return bool_variable_expr_gen(e->variable, true, false); break;
  case E_BINARY: return bool_binary_expr_gen(e->binary, true, false); break;
...
void if_stmt_gen(if_stmt *s) {
  ...
  if(typecheck(s->test)!=TY_BOOLEAN){
    error("Type mismatch error");
  }
...
void assign_stmt_gen(assign_stmt *s) {
  ...
  case TY_INTEGER:
    ...
    if(typecheck(s->rvalue) != TY_INTEGER){
      error("Type mismatch error");
    }
  ...
  case TY_BOOLEAN:
    ...
    if(typecheck(s->rvalue) != TY_BOOLEAN){
      error("Type mismatch error");
    }
    quads[next].opcode = O_NOOP;
    quads[next++].left = true;
...
/**
 * Generate code for a write list.
 * @param l The list to write.
 */
void write_list_gen(write_list *l){
  if(l){
    symbol *left;
    symbol *right;
    left = expr_gen(l->arg);
    if(l->arg->tag == E_BOOL_CONST /* l->arg->tag == E_VARIABLE*/) {
      if(l->arg->tag == E_BOOL_CONST) {
        if(l->arg->bool_const->value == 0) {
         quads[next].opcode = O_FALSE;
        }else if(l->arg->bool_const->value == 1) {
          quads[next].opcode = O_TRUE;
        }
      } /*else if(l->arg->tag == E_VARIABLE) {
        if(l->arg->variable->symbol->value == 0) {
         quads[next].opcode = O_FALSE;
        }else if(l->arg->variable->symbol->value == 1) {
          quads[next].opcode = O_TRUE;
        }
      }*/
    }else{
      typecheck(l->arg);
      if(l->field_width != NULL){
        right = expr_gen(l->field_width);
      } else {
...
void write_stmt_gen(write_stmt *s) {
  write_list_gen(s->arg);
}
/**
 * Generate code for a write list.
 * @param l The list to write.
 */
void read_list_gen(read_list *l) {
  if(l) {
    symbol *left = expr_gen(l->arg);
    quads[next].opcode = O_READ;
    quads[next++].left = left;
    read_list_gen(l->next);
  }
}
/**
 ...
 * @param s The statement
 */
void read_stmt_gen(read_stmt *s) {
 read_list_gen(s->arg); 
}
...
void stmt_gen(stmt *s) {
  ...
  case S_READ: read_stmt_gen(s->read); break;
