...
%right T_SNLNOT
%token T_LPAREN T_RPAREN T_ASSIGN T_SEMI T_PERIOD T_COLON T_COMMA T_LBRACK T_RBRACK
%token T_VAR T_INTEGER T_NOT T_BOOLEAN
%token T_IF T_THEN T_ELSE T_WHILE T_DO
%token T_PROGRAM T_BEGIN T_END
%token T_WRITE T_WRITELN T_READ
%token T_PP T_MM
...
write_list :
  expr { $$ = write_list_new($1, NULL, NULL); }
|  expr T_COMMA write_list { $$ = write_list_new($1, NULL, $3); }
|  expr T_COLON expr { $$ = write_list_new($1, $3, NULL); }
|  expr T_COLON expr T_COMMA write_list { $$ = write_list_new($1, $3, $5); }
;

write_stmt :
  T_WRITE T_LPAREN write_list T_RPAREN { $$ = write_stmt_new($3); }
;

writeln_stmt :
  T_WRITELN T_LPAREN T_RPAREN { $$ = writeln_stmt_new(NULL); }
|  T_WRITELN T_LPAREN write_list T_RPAREN { $$ = writeln_stmt_new($3); }
;

read_list :
  expr { $$ = read_list_new($1, NULL); }
|  expr T_COMMA read_list { $$ = read_list_new($1, $3); }
;
read_stmt :
  T_READ T_LPAREN read_list T_RPAREN { $$ = read_stmt_new($3); }
;
empty_stmt :
  { $$ = empty_stmt_new(); }
;
