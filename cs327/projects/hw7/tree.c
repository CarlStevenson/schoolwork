...
/**
 * Create a write list to be outputted.
 * @param arg The argument.
 * @param field_width The inteded field width.
 * @param next The rest of the write list.
 * @return The new statement.
 */
write_list *write_list_new(expr *arg, expr *field_width, write_list *next) {
  write_list *s = malloc(sizeof(write_list));
  s->arg = arg;
 ...
 * @return The new statement.
 */
write_stmt *write_stmt_new(write_list *arg) {
 ...
 * @return The new statement.
 */
writeln_stmt *writeln_stmt_new(write_list *arg) {
...
/**
 * Create a read list to be assigned values.
 * @param arg The argument.
 * @param next The rest of the list.
 * @return The new list.
 */
read_list *read_list_new(expr *arg, read_list *next) {
  read_list *s = malloc(sizeof(read_list));
  s->arg = arg;
  s->next = next;
  return s;
}
 ...
 * @param arg The argument.
 */
read_stmt *read_stmt_new(read_list *arg) {
  read_stmt *s = malloc(sizeof(read_stmt));
  s->arg = arg;
  return s;
}
...
stmt *stmt_new(stmt_tag tag, ... ) {
  ...
  case S_READ: s->read = va_arg(args, read_stmt *); break;
...
void stmt_print(stmt *s, int indent) {
  ...
  case S_WRITE:
    ...
    expr_print(s->write->arg->arg, indent + INDENT);
    break;  
  case S_WRITELN:
